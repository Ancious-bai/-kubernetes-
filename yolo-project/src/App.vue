...
const filterTrainLog=log=>{if(!log)return'';if(/排队中|QUEUED|等待调度/.test(log))return'⏳ 排队中，等待资源分配...';let c=log.replace(/\x1b\[[0-9;]*[a-zA-Z]/g,'\n');c=c.replace(/\[K/g,'\n');c=c.replace(/\[[\d;]*m/g,'');c=c.replace(/[\u2500-\u257F━╸╺╻╹╿╽╼╾─━]/g,' ');const segments=c.split('\n');const lastLines=[];const seen=new Set();for(const s of segments){const t=s.trim();if(t&&!seen.has(t)){seen.add(t);lastLines.push(t)}}const rows=[];const used=new Set();let started=false;for(let i=0;i<lastLines.length;i++){const t=lastLines[i];if(/Starting training/.test(t))started=true;const em=t.match(/^(\d+\/\d+)\s+\d*G?\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+\d+\s+\d+\s*:\s*100%/);if(em){for(let j=i+1;j<Math.min(i+10,lastLines.length);j++){const vm=lastLines[j].match(/^all\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);if(vm){rows.push({e:em[1],i:vm[1],n:vm[2],p:vm[3],r:vm[4],m5:vm[5],m9:vm[6]});used.add(j);break}if(/^\d+\/\d+/.test(lastLines[j]))break}}}let ci=-1;for(let i=0;i<lastLines.length;i++)if(/epochs?\s+completed/.test(lastLines[i])){ci=i;break}if(ci>=0){for(let j=ci+1;j<lastLines.length;j++){const vm=lastLines[j].match(/^all\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);if(vm&&!used.has(j)){rows.push({e:'best',i:vm[1],n:vm[2],p:vm[3],r:vm[4],m5:vm[5],m9:vm[6]});break}if(/Speed:/.test(lastLines[j]))break}}if(!rows.length){if(started)return'🔄 训练中，等待第1轮epoch完成...';return'⏳ 等待训练启动...'}let res='Epoch    Class  Images  Instances  Box(P)      R        mAP50    mAP50-95\n';res+='─'.repeat(72)+'\n';for(const d of rows)res+=`${String(d.e).padEnd(8)}all    ${String(d.i).padStart(5)}  ${String(d.n).padStart(9)}  ${String(d.p).padStart(9)}  ${String(d.r).padStart(7)}  ${String(d.m5).padStart(7)}  ${String(d.m9)}\n`;return res.trim()}
