...
const renderTrainLog=log=>{const d=parseTrainLog(log);if(d.status==='queued')return'<div class="tl-status">⏳ 排队中，等待资源分配...</div>';if(d.status==='waiting')return'<div class="tl-status">⏳ 等待训练启动...</div>';if(d.status==='empty')return'';let h='';if(d.header){h+=`<pre class="tl-header">${d.header.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>`}if(d.progress){h+=`<div class="tl-progress">🔄 Epoch ${d.progress.e}: <span class="tl-pct">${d.progress.pct}%</span> | box_loss: ${d.progress.box}  cls_loss: ${d.progress.cls}  dfl_loss: ${d.progress.dfl}</div>`}else if(d.rows.length===0&&d.status==='training'){h+='<div class="tl-status">🔄 训练中，等待第1轮epoch完成...</div>'}if(d.rows.length>0){h+='<table class="tl-table"><tr><th>Epoch</th><th>Class</th><th>Images</th><th>Instances</th><th>Box(P)</th><th>R</th><th>mAP50</th><th>mAP50-95</th></tr>';for(const r of d.rows){const cls=r.e==='best'?'tl-best':'';h+=`<tr class="${cls}"><td>${r.e}</td><td>all</td><td>${r.i}</td><td>${r.n}</td><td>${r.p}</td><td>${r.r}</td><td>${r.m5}</td><td>${r.m9}</td></tr>`}h+='</table>'}return h}
