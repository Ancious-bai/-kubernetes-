<template>
  ...
  <div v-else class="app-layout">
      ...
      <div v-show="activePage==='models'" class="page-models">
        ...
        <el-card shadow="hover" style="margin-top:14px">
          <div style="overflow-x:auto">
            <table class="records-table">
              <thead><tr>
                <th>模型名称</th><th>推理数据集</th><th>状态</th><th>提交者</th><th>创建时间</th><th>操作</th>
              </tr></thead>
              <tbody>
                <tr v-for="inf in inferenceList" :key="inf.id">
                  <td><span style="font-weight:600">{{ inf.modelName }}</span></td>
                  <td><span>{{ inf.dataName }}</span></td>
                  <td><el-tag :type="inf.status==='completed'?'success':(inf.status==='running'?'warning':(inf.status==='failed'?'danger':'info'))" size="small">{{ inf.status==='completed'?'已完成':(inf.status==='running'?'运行中':(inf.status==='failed'?'失败':'排队中')) }}</el-tag></td>
                  <td><span>{{ inf.createdBy }}</span></td>
                  <td><span class="param-fixed">{{ inf.createdAt }}</span></td>
                  <td><div class="record-actions">
                    <el-button size="small" type="primary" @click="viewInferenceLog(inf)">推理日志</el-button>
                    <el-button v-if="inf.status==='completed'" size="small" type="success" @click="viewInferenceResult(inf)">推理结果</el-button>
                ...
                <tr v-if="!inferenceList.length"><td colspan="6" style="color:#909399">暂无推理记录</td></tr>
              </tbody>
            </table>
          </div>
        </el-card>
            ...
            <table class="records-table" style="font-size:12px">
              ...
              <tbody>
                <tr v-for="pod in filteredSystemPods" :key="pod.podName">
                  ...
                  <td><span>{{ formatResourceValue(pod.cpuRequest,2) }} / {{ pod.cpuLimit>0?formatResourceValue(pod.cpuLimit,2):'未限制' }}</span></td>
                  <td><span>{{ formatResourceValue(pod.memRequestMB,0) }} / {{ pod.memLimitMB>0?formatResourceValue(pod.memLimitMB,0):'未限制' }}</span></td>
    ...
    <el-dialog :title="'推理结果 - ' + currentInferenceResultTitle" v-model="inferenceResultVisible" width="1000px" :close-on-click-modal="false">
      ...
      <div v-else>
        <div v-if="predictImages.length">
          <h4 style="margin:0 0 10px;font-size:14px;color:#303133">推理结果 ({{ predictImages.length }}张) - 点击查看大图</h4>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;max-height:500px;overflow:auto;padding:2px">
            <div v-for="img in predictImages" :key="img.path" style="border:1px solid #dcdfe6;border-radius:8px;overflow:hidden;cursor:pointer;background:#fff;transition:all .2s" @click="openInferenceResultImage(img.path)" @mouseenter="$event.currentTarget.style.borderColor='#409eff';$event.currentTarget.style.boxShadow='0 2px 12px rgba(64,158,255,.2)'" @mouseleave="$event.currentTarget.style.borderColor='#dcdfe6';$event.currentTarget.style.boxShadow='none'">
              <img :src="getInferenceResultImageUrl(img.path)" style="width:100%;height:150px;object-fit:contain;display:block;background:#1a1a1a" />
...
const predictImages=computed(()=>(inferenceResultDetailData.value.files||[]).filter(f=>f.isImage&&!f.isDirectory&&f.name.endsWith('_predict.png')))
...
const syncNodes=async()=>{nodesSyncing.value=true;try{const res=await api.value.post('/api/nodes/sync');await loadClusterNodes();await loadClusterOverview();if(res.data.status==='error'){showMsg(res.data.message||'同步失败','error')}else{showMsg(res.data.message||`同步完成，共${res.data.syncedCount||0}个节点`)}}catch(e){showMsg('同步失败:'+e.message,'error')}finally{nodesSyncing.value=false}}
...
const formatResourceValue=(v,digits)=>{if(v==null||v===undefined||v==='')return '0';return Number(v).toFixed(digits)}
...
const checkInferenceStatusChanges=async()=>{try{const newRecords=(await api.value.get('/api/models/inferences')).data;let changed=false;for(const inf of newRecords){const oldInf=oldInferenceRecords.find(o=>o.id===inf.id);if(oldInf&&oldInf.status!==inf.status){changed=true;if((oldInf.status==='running')&&(inf.status==='completed')){showMsg(`推理 ${inf.modelName}→${inf.dataName} 已完成`)}else if((oldInf.status==='running')&&(inf.status==='failed')){showMsg(`推理 ${inf.modelName}→${inf.dataName} 失败`,'error')}if(inf.status!=='running'){const logKey=`${inf.modelName}_predict_${inf.dataName}`;if(predictLogs.value[logKey]){predictLogs.value[logKey]+='\n\n[推理已结束]';delete predictLogs.value[logKey]}}}}oldInferenceRecords=newRecords;inferenceList.value=newRecords}catch(e){}}
...
const loadInferenceRecords=async()=>{try{const params={};if(inferenceFilter.modelName)params.modelName=inferenceFilter.modelName;if(inferenceFilter.dataName)params.dataName=inferenceFilter.dataName;const r=await api.value.get('/api/models/inferences',{params});inferenceList.value=r.data;oldInferenceRecords=[...r.data]}catch(e){}}
