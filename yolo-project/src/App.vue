...
const filterTrainLog=log=>{if(!log)return'';const lines=log.split('\n');const keep=new Set();for(let i=0;i<lines.length;i++){const line=lines[i];const trimmed=line.trim();if(!trimmed){keep.add(i);continue}if(/Epoch\s+GPU_mem\s+box_loss/.test(trimmed)){keep.add(i);continue}if(/Class\s+Images\s+Instances/.test(trimmed)){keep.add(i);if(i+1<lines.length)keep.add(i+1);continue}if(/^\s*\d+\/\d+\s+\d*G?\s/.test(trimmed)&&/100%/.test(trimmed)){for(let j=i-1;j>=0;j--){if(/Epoch\s+GPU_mem\s+box_loss/.test(lines[j].trim())){keep.add(j);break}if(lines[j].trim())break}keep.add(i);continue}if(/^(all|Class)\s+\d+/.test(trimmed)){keep.add(i);continue}if(/epochs? completed/.test(trimmed)){keep.add(i);continue}if(/Speed:/.test(trimmed)){keep.add(i);continue}if(/训练参数|输出目录|正在训练/.test(trimmed)){keep.add(i);continue}if(/Ultralytics\s+\d/.test(trimmed)){keep.add(i);continue}if(/optimizer:/i.test(trimmed)&&/AdamW|SGD|Adam/.test(trimmed)){keep.add(i);continue}if(/Image sizes/.test(trimmed)){keep.add(i);continue}if(/Starting training/.test(trimmed)){keep.add(i);continue}if(/error|Error|FAIL|Traceback/i.test(trimmed)){keep.add(i);continue}}return lines.filter((_,i)=>keep.has(i)).join('\n')}
