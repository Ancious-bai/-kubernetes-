...
const filterTrainLog=log=>{if(!log)return'';let c=log.replace(/\x1b\[[0-9;]*[a-zA-Z]/g,'');c=c.replace(/\[\d+m/g,'');c=c.replace(/\[K/g,'');const ls=c.split('\n');const rows=[];const used=new Set();for(let i=0;i<ls.length;i++){const t=ls[i].replace(/━/g,'').replace(/╸/g,'').replace(/─/g,'').trim();const em=t.match(/^(\d+\/\d+)\s+\d*G?\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+\d+\s+\d+\s*:\s*100%/);if(em){for(let j=i+1;j<Math.min(i+20,ls.length);j++){const vl=ls[j].replace(/━/g,'').replace(/╸/g,'').replace(/─/g,'').trim();const vm=vl.match(/^all\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);if(vm){rows.push({e:em[1],i:vm[1],n:vm[2],p:vm[3],r:vm[4],m5:vm[5],m9:vm[6]});used.add(j);break}if(/^\d+\/\d+/.test(vl))break}}}let ci=-1;for(let i=0;i<ls.length;i++)if(/epochs?\s+completed/.test(ls[i])){ci=i;break}if(ci>=0){for(let j=ci+1;j<ls.length;j++){const vl=ls[j].replace(/━/g,'').replace(/╸/g,'').replace(/─/g,'').trim();const vm=vl.match(/^all\s+(\d+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);if(vm&&!used.has(j)){rows.push({e:'best',i:vm[1],n:vm[2],p:vm[3],r:vm[4],m5:vm[5],m9:vm[6]});break}}}if(!rows.length)return'训练中，等待epoch完成...';let res='Epoch    Class  Images  Instances  Box(P)      R        mAP50    mAP50-95\n';res+='─'.repeat(72)+'\n';for(const d of rows)res+=`${String(d.e).padEnd(8)}all    ${String(d.i).padStart(5)}  ${String(d.n).padStart(9)}  ${String(d.p).padStart(9)}  ${String(d.r).padStart(7)}  ${String(d.m5).padStart(7)}  ${String(d.m9)}\n`;return res.trim()}
